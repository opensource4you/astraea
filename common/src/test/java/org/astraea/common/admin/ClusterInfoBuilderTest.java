...
class ClusterInfoBuilderTest {
  ...
  @Test
  void addNode() {
    Assertions.assertEquals(Set.of(), ClusterInfoBuilder.builder().addNode().build().nodes());
    Assertions.assertEquals(
        Set.of(1, 2, 3, 4, 5, 100),
        ClusterInfoBuilder.builder().addNode(List.of(1, 2, 3, 4, 5, 100)).build().nodes().stream()
            .map(NodeInfo::id)
            .collect(Collectors.toSet()));
    Assertions.assertEquals(
        ClusterInfoBuilder.builder().addNode(List.of(1, 2, 3)).build().nodes(),
        ClusterInfoBuilder.builder().addNode(1, 2, 3).build().nodes(),
  ...
  void addFolders() {
    ...
    Assertions.assertEquals(
        ...
        ClusterInfoBuilder.builder()
            ...
            .addFolders(2, "/ssd1", "/ssd2", "/hdd3")
            .addFolders(3, "/hdd1", "/hdd2", "/hdd3")
            .build()
            .brokerFolders());
  }
  ...
  void addTopic() {
    ...
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> ClusterInfoBuilder.builder().addNode(1).addTopic("topic", 1, (short) 2).build(),
        "Insufficient nodes");
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> ClusterInfoBuilder.builder().addNode(1).addTopic("topic", 1, (short) 1).build(),
        "No folder info");

    var cluster =
        ClusterInfoBuilder.builder()
            .addNode(1, 2, 3)
            .addFolders(1, "/broker1/ssd1", "/broker1/ssd2", "/broker1/ssd3")
            .addFolders(2, "/broker2/ssd1", "/broker2/ssd2", "/broker2/ssd3")
            .addFolders(3, "/broker3/ssd1", "/broker3/ssd2", "/broker3/ssd3")
  ...
  void mapTopic() {
    ...
    var cluster2 =
        ClusterInfoBuilder.builder(cluster1)
            .mapTopic("topic", r -> Replica.builder(r).lag(5566).build())
            .build();
    Assertions.assertTrue(
        cluster2
            .replicaStream()
  ...
  void mapTopicPartition() {
    ...
    var cluster2 =
        ClusterInfoBuilder.builder(cluster1)
            .mapTopicPartition(
                TopicPartition.of("topic", 3), r -> Replica.builder(r).lag(5566).build())
            .build();
    Assertions.assertTrue(
        cluster2
            .replicaStream()
            .allMatch(i -> (i.partition() == 3) ? (i.lag() == 5566) : (i.lag() == 0)));
  }
