...
/**
 * Record current set. The method `measure` will return the number of increased element compare to
 * the last `interval` record.
 */
public class SetIncreased<V> implements Stat<Set<V>, Integer> {
  private final LinkedList<Set<V>> past = new LinkedList<>();

  private final int interval;

  public SetIncreased(int interval) {
  ...
  public synchronized Integer measure() {
    ...
    return (int) current.stream().filter(e -> !last.contains(e)).count();
  }
}
