...
@FunctionalInterface
public interface SSerializer<T> {
  ...
  static <T> org.apache.kafka.common.serialization.Serializer<T> of(SSerializer<T> serializer) {
  ...
  private static <T> SSerializer<T> of(
  ...
  SSerializer<byte[]> BYTE_ARRAY = of(new ByteArraySerializer());
  SSerializer<String> STRING = of(new StringSerializer());
  SSerializer<Integer> INTEGER = of(new IntegerSerializer());
  SSerializer<Long> LONG = of(new LongSerializer());
  SSerializer<Float> FLOAT = of(new FloatSerializer());
  SSerializer<Double> DOUBLE = of(new DoubleSerializer());
  SSerializer<NodeInfo> NODE_INFO = new NodeInfoSerializer();
  SSerializer<Topic> TOPIC = new TopicSerializer();
  SSerializer<Replica> REPLICA = new ReplicaSerializer();
  SSerializer<ClusterInfo> CLUSTER_INFO = new ClusterInfoSerializer();
   ...
   */
  static <T> SSerializer<T> of(TypeRef<T> typeRef) {
    return new JsonSerializer<>(typeRef);
  }

  class JsonSerializer<T> implements SSerializer<T> {
  ...
  class NodeInfoSerializer implements SSerializer<NodeInfo> {
    ...
    public byte[] serialize(String topic, Collection<Header> headers, NodeInfo data) {
      var nodeInfoSize =
          4 // [id]
              + 2 // [host length]
              + OldByteUtils.toBytes(data.host()).length // [host]
              + 4; // [port]
      var buffer = ByteBuffer.allocate(nodeInfoSize);
      buffer.putInt(data.id());
      OldByteUtils.putLengthString(buffer, data.host());
      buffer.putInt(data.port());
      return buffer.array();
    }
  }

  class TopicSerializer implements SSerializer<Topic> {
    ...
    public byte[] serialize(String topic, Collection<Header> headers, Topic data) {
      var configSize =
          data.config().raw().entrySet().stream()
              .mapToInt(
                  entry ->
                      2
                          + OldByteUtils.toBytes(entry.getKey()).length
                          + 2
                          + OldByteUtils.toBytes(entry.getValue()).length)
              .sum();
      var topicSize =
          2 // [topic length]
              + OldByteUtils.toBytes(data.name()).length // [topic]
              + 4 // [nums of config]
              + configSize // [config]
              + 1 // [internal]
      ...
      OldByteUtils.putLengthString(buffer, data.name());
      buffer.putInt(data.config().raw().size());
      data.config()
          .raw()
          .forEach(
              (key, value) -> {
                OldByteUtils.putLengthString(buffer, key);
                OldByteUtils.putLengthString(buffer, value);
              });
      buffer.put(OldByteUtils.toBytes(data.internal()));
  ...
  class ReplicaSerializer implements SSerializer<Replica> {

    @Override
    public byte[] serialize(String topic, Collection<Header> headers, Replica data) {
      var nodeInfo = SSerializer.NODE_INFO.serialize(topic, headers, data.nodeInfo());
      var replicaSize =
          2 // [topic length]
              + OldByteUtils.toBytes(topic).length // [topic]
              + 4 // [partition]
              + 4 // [nodeInfo length]
              ...
              + 1 // [isPreferredLeader]
              + 2 // [path length]
              + OldByteUtils.toBytes(data.path()).length; // [path]
      var buffer = ByteBuffer.allocate(replicaSize);
      OldByteUtils.putLengthString(buffer, topic);
      buffer.putInt(data.partition());
      buffer.putInt(nodeInfo.length);
      buffer.put(nodeInfo);
      buffer.putLong(data.lag());
      buffer.putLong(data.size());
      buffer.put(OldByteUtils.toBytes(data.isLeader()));
      buffer.put(OldByteUtils.toBytes(data.isSync()));
      buffer.put(OldByteUtils.toBytes(data.isFuture()));
      buffer.put(OldByteUtils.toBytes(data.isOffline()));
      buffer.put(OldByteUtils.toBytes(data.isPreferredLeader()));
      OldByteUtils.putLengthString(buffer, data.path());
      return buffer.array();
    }
  }

  class ClusterInfoSerializer implements SSerializer<ClusterInfo> {

    @Override
    public byte[] serialize(String topic, Collection<Header> headers, ClusterInfo data) {
      var nodes =
          data.nodes().stream()
              .map(nodeInfo -> SSerializer.NODE_INFO.serialize(topic, headers, nodeInfo))
              .collect(Collectors.toList());
      var topics =
          data.topics().values().stream()
              .map(t -> SSerializer.TOPIC.serialize(topic, headers, t))
              .collect(Collectors.toList());
      var replicas =
          data.replicas().stream()
              .map(replica -> SSerializer.REPLICA.serialize(replica.topic(), headers, replica))
              .collect(Collectors.toList());
      var clusterInfoSize =
          2 // [clusterId length]
              + OldByteUtils.toBytes(data.clusterId()).length // [clusterId]
              + 4 // [nums of nodeInfo]
      ...
      OldByteUtils.putLengthString(buffer, data.clusterId());
  ...
  class BeanObjectSerializer implements SSerializer<BeanObject> {
    @Override
