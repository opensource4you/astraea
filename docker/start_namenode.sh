...
declare -r NAMENODE_HTTP_ADDRESS="${NAMENODE_HTTP_ADDRESS:-"$(getRandomPort)"}"
...
declare -r JMX_OPTS="-Dcom.sun.management.jmxremote \
  ...
  -Dcom.sun.management.jmxremote.port=$NAMENODE_JMX_PORT \
  -Dcom.sun.management.jmxremote.rmi.port=$NAMENODE_JMX_PORT \
...
docker run -d --init \
  ...
  -p $NAMENODE_JMX_PORT:$NAMENODE_JMX_PORT \
  -p $NAMENODE_PORT:8020 \
  -p $EXPORTER_PORT:$EXPORTER_PORT \
  "$IMAGE_NAME" /bin/bash -c "./bin/hdfs namenode -format && ./bin/hdfs namenode"

echo "================================================="
echo "http address: ${ADDRESS}:$NAMENODE_HTTP_ADDRESS"
echo "jmx address: ${ADDRESS}:$NAMENODE_JMX_PORT"
echo "exporter address: ${ADDRESS}:$EXPORTER_PORT"
echo "run $DOCKER_FOLDER/start_datanode.sh fs.defaultFS=hdfs://${ADDRESS}:$NAMENODE_PORT to join datanode"
echo "================================================="