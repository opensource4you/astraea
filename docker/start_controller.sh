...
declare -r KAFKA_VERSION=${KAFKA_REVISION:-${KAFKA_VERSION:-4.1.0}}
...
function showHelp() {
  ...
  echo "    KAFKA_VERSION=4.1.0                            set version of kafka distribution"
...
function generateDockerfileByVersion() {
  ...
  if [[ "$KAFKA_VERSION" == *"rc"* ]]; then
    ## `4.1.0-rc1` the rc release does not exist in archive repo
