...
public class TopicExplorer {
  ...
  static class PartitionInfo {
    ...
    public PartitionInfo(TopicPartition topicPartition, long earliestOffset, long latestOffset, List<Group> consumerGroups, List<Replica> replicas) {
      this.topicPartition = topicPartition;
      this.earliestOffset = earliestOffset;
      this.latestOffset = latestOffset;
      this.consumerGroups = consumerGroups.stream()
    ...
    Function<String, List<Integer>> partitionsOf = (topicName) ->
    ...
    return topics.stream()
            .collect(Collectors.toMap(Function.identity(), (topic) -> {
  ...
  static class TreeOutput {
      ...
      public void print() {
          ...
          nextLevel(" ", () -> {
              info.forEach((topic, partitionInfos) -> {
      ...
      private void printConsumerGroup(String topic, List<PartitionInfo> partitionInfos) {
          ...
          nextLevel(NEXT_LEVEL, () -> {
              ...
              var consumerGroups = partitionInfos.stream()
                  ...
                  consumerGroups
                          .forEach(groupId -> {
                              ...
                              nextLevel("  ", () -> {
                                  ...
                                  var groups = partitionInfos.stream()
                                  ...
                                  IntStream.range(0, partitionInfos.size())
                                          .mapToObj(x -> new ConsumeProgress(
                                                  topic,
                                                  x,
                                                  groups.get(x),
                                                  info))
                                  ...
                                  treePrintln("Members:");
                                  nextLevel("  ", () -> {
                                      var dutyOfMembers = groups.values().stream()
                                              .flatMap(x -> x.members().stream())
                                              .distinct()
                                              .collect(Collectors.toMap(
                                                      (m) -> m,
                                                      (m) -> groups.entrySet().stream()
                                                              .filter(g -> g.getValue().members().contains(m))
                                                              .map(Map.Entry::getKey)
                                                              .collect(Collectors.toList())
                                              ));

                                      if(dutyOfMembers.isEmpty())
                                          ...
                                          dutyOfMembers.keySet().stream()
                                                  ...
                                                  .forEach(member -> {
                                                      ...
                                                      nextLevel(NEXT_LEVEL, () -> {
                                                          treePrintln("working on partition %s",
                                                                  dutyOfMembers.get(member)
                                                                          .stream()
                                                                          .sorted()
                                                                          .map(Object::toString)
                                                          ...
                                                          if(member.groupInstanceId().isPresent())
                                                              treePrintln("groupInstanceId: \"%s\"", member.groupInstanceId().get());
                                                          else
                                                              treePrintln("groupInstanceId: none");
                                                      });
                                                  });
                                      }
                                  });
      ...
      private void printPartitionReplica(List<PartitionInfo> partitionInfos) {
          ...
          nextLevel(NEXT_LEVEL, () -> {
              ...
              partitionInfos.forEach(partitionInfo -> {
                  treePrintln("Partition \"%d\" (offset range: [%d, %d])",
                  ...
                  nextLevel("  ", () -> {
                      ...
                      nextLevel(NEXT_LEVEL, () -> {
                          partitionInfo.replicas.stream().sorted(Comparator.comparing(Replica::broker)).forEach(replica -> {
                              treePrintln("replica on broker %-4s %17s %s",
                                      "#" + replica.broker(),
      ...
      private static class ConsumeProgress {
          ...
          private ConsumeProgress(String topic, int partition, Group group, Map<String, List<PartitionInfo>> map) {
              this.topic = topic;
          ...
          private String progressBar() {
              int totalBlocks = 20;
              int filledBlocks = Math.min((int) (20.0 * ((double)(current() - earliest()) / (latest() - earliest()))), 20);
          ...
          public String toString() {
              ...
              return String.format("consume progress of partition %" + partitionDigits + "d %s (earliest/current/latest offset %d/%d/%d)",
                      index,
                      progressBar(),
                      earliest(),
                      current(),
      ...
      private static class ReplicaHelper {
          static String dataSizeString(DataSize dataSize) {
              return String.format("%.2f %s",
                      dataSize.idealMeasurement().doubleValue(),
                      dataSize.idealDataUnit());
          }
