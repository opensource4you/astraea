...
public class Network extends TimerTask {
  ...
  public Network(String ip, String controlPlan){
    ...
    this.nodesRequest = HttpRequest.newBuilder().uri(URI.create("http://" + ip + "/ws/v1/nodes")).GET().build();
    this.appsRequest = HttpRequest.newBuilder().uri(URI.create("http://" + ip + "/ws/v1/apps")).GET().build();
    this.ip = ip;
  ...
  public void run()   {
    ...
    try {
      HttpResponse<String> response = client.send(nodesRequest, HttpResponse.BodyHandlers.ofString());
  ...
  public List<Node> getNodes(String body, String controlPlan) {
    ...
    try {
      ...
      for (int i = 0; i < json.getJSONObject(0).getJSONArray(NODE_KEY).length(); i++) {
        if (json.getJSONObject(0).getJSONArray(NODE_KEY).getJSONObject(i).get(Info.NODEID).toString().compareTo(controlPlan)==0)
          continue;

        var nodeID = json.getJSONObject(0).getJSONArray(NODE_KEY).getJSONObject(i).get(Info.NODEID).toString();
        var hostName = json.getJSONObject(0).getJSONArray(NODE_KEY).getJSONObject(i).get(Node.HOSTNAME_KEY).toString();
        var rackName = json.getJSONObject(0).getJSONArray(NODE_KEY).getJSONObject(i).get(Node.RACKNAME_KEY).toString();

        Map<String, BigInteger> capacity = unmarshall(json.getJSONObject(0).getJSONArray(NODE_KEY).getJSONObject(i).getJSONObject(Node.CAPACITY_KEY));
        Map<String, BigInteger> available = unmarshall(json.getJSONObject(0).getJSONArray(NODE_KEY).getJSONObject(i).getJSONObject(Node.AVAILABLE_KEY));
  ...
  public List<Application> getApps(String body) {
    ...
    try {
      ...
      for(int i = 0;i< json.length();i++){
        var applicationState = json.getJSONObject(i)
                .get("applicationState")
                .toString();
        if (applicationState.compareTo("New")==0||applicationState.compareTo("Accepted")==0){
          continue;
        }
        var applicationID = json.getJSONObject(i)
                .get("applicationID")
                .toString();
        var partition = json.getJSONObject(i)
                .get("partition")
                .toString();
        var queueName = json.getJSONObject(i)
                .get("queueName")
                .toString();

        var tmp = json.getJSONObject(i)
                .get("submissionTime")
                .toString();
        ...
        var app = new Application(applicationID, partition, queueName, submissionTime.longValue(), finishedTime.longValue(), applicationState);
        apps.add(app);
      }
    } catch (JSONException e) {
      e.printStackTrace();
