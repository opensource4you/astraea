...
public class Info extends Collector {
    ...
    public void update(List<Node> nodes){
        this.nodes = nodes;
        var averageCore = nodes.stream()
                .map(node -> node.getAvailable()
                        .get(NodeSortingPolicy.CORE_KEY))
                .reduce(BigInteger.ZERO, (n1, n2)-> n1.add(n2))
                .divide(BigInteger.valueOf(nodes.stream()
                        .count()));
        var averageMemory = nodes.stream()
                .map(node -> node.getAvailable()
        ...
        var maxCore = nodes.stream()
                .map(node -> node.getAvailable()
                        .get(NodeSortingPolicy.CORE_KEY))
                .max(Comparator.comparing(BigInteger::toString))
                .get();
        var maxMemory = nodes.stream()
                .map(node -> node.getAvailable()
                        .get(NodeSortingPolicy.MEMORY_KEY))
                .max(Comparator.comparing(BigInteger::toString))
                .get();
        ...
        var minCore =nodes.stream()
                .map(node -> node.getAvailable()
                        .get(NodeSortingPolicy.CORE_KEY))
                .max(Comparator.comparing(BigInteger::toString))
                .get();
        var minMemory =nodes.stream()
                .map(node -> node.getAvailable()
                        .get(NodeSortingPolicy.MEMORY_KEY))
                .max(Comparator.comparing(BigInteger::toString))
                .get();
    ...
    public List<MetricFamilySamples> collect() {
        ...
        // With no labels.
        mfs.add(new GaugeMetricFamily(MAX_CORE, "help", maxSource.get(NodeSortingPolicy.CORE_KEY).doubleValue()));
        mfs.add(new GaugeMetricFamily(MAX_MEMORY, "help", maxSource.get(NodeSortingPolicy.MEMORY_KEY).doubleValue()));
        mfs.add(new GaugeMetricFamily(MIN_CORE, "help", minSource.get(NodeSortingPolicy.CORE_KEY).doubleValue()));
        mfs.add(new GaugeMetricFamily(MIN_MEMORY, "help", minSource.get(NodeSortingPolicy.MEMORY_KEY).doubleValue()));
        mfs.add(new GaugeMetricFamily(AVERAGE_CORE, "help", averageSource.get(NodeSortingPolicy.CORE_KEY).doubleValue()));
        mfs.add(new GaugeMetricFamily(AVERAGE_MEMORY, "help", averageSource.get(NodeSortingPolicy.MEMORY_KEY).doubleValue()));
        mfs.add(new GaugeMetricFamily(SD_CORE, "help", SDSource.get(NodeSortingPolicy.CORE_KEY).doubleValue()));
        mfs.add(new GaugeMetricFamily(SD_MEMORY, "help", SDSource.get(NodeSortingPolicy.MEMORY_KEY).doubleValue()));

        // With labels
        GaugeMetricFamily labeledGaugeCore = new GaugeMetricFamily(NODES_CORE, "help", Arrays.asList(NODEID));
        nodes.stream()
                .forEach(node -> labeledGaugeCore.addMetric(Arrays.asList(node.getNodeID()), node.getAvailable().get(NodeSortingPolicy.CORE_KEY).doubleValue()));
        mfs.add(labeledGaugeCore);
        GaugeMetricFamily labeledGaugeMemory = new GaugeMetricFamily(NODES_MEMORY, "help", Arrays.asList(NODEID));
        nodes.stream()
                .forEach(node -> labeledGaugeMemory.addMetric(Arrays.asList(node.getNodeID()), node.getAvailable().get(NodeSortingPolicy.MEMORY_KEY).doubleValue()));
