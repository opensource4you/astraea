...
public class Info extends Collector {
  ...
  public void update(List<Node> nodes, List<Application> apps) throws IOException {
    ...
    source.put(NodeSortingPolicy.CORE_KEY, (1-core.doubleValue()/capacityCore.doubleValue())*100);
    source.put(NodeSortingPolicy.MEMORY_KEY, (1-memory.doubleValue()/capacityMemory.doubleValue())*100);
    ...
    this.apps = apps;
    this.runningApps = apps.stream().filter(app -> app.getApplicationState().compareTo("Running")==0).count();

    completedApps = Long.valueOf(apps.stream().filter(app -> app.getApplicationState().compareTo("Completed")==0).count());
    if(completedApps!=0){
      this.delayTime = apps.stream().filter(app -> app.getApplicationState().compareTo("Completed")==0).map(app -> app.getFinishedTime()-app.getSubmissionTime())
              .reduce(Long.valueOf(0), (total, app)->total + app)/completedApps;
  ...
  public List<MetricFamilySamples> collect() {
    ...
    mfs.add(
            new GaugeMetricFamily(COMPLETED_APPS
                    , "help",completedApps));
    // With no labels.
    mfs.add(
    ...
    mfs.add(
            new GaugeMetricFamily(
                    VCORE_UTILIZATION, "help", source.get(NodeSortingPolicy.CORE_KEY)));
    mfs.add(
            new GaugeMetricFamily(
                    RUNNING_APPS, "help", runningApps));

    mfs.add(
            new GaugeMetricFamily(
                    DELAY_TIME, "help", delayTime));
